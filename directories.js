...
if (process.argv.includes('regenerate')) {
   setImmediate(async () => {
      try {
         const sourceCode = (await fs.readFile(import.meta.filename, 'utf-8')).replace(
         ...
         const generatedCode = await Promise.all(
            sources.map(async ([constName, old, ...args]) => {
               ...
               return `\r\nexpot const ${constName} = ${JSON.stringify(result, null, 3)}\r\n`;
            }),
         );

         await fs.writeFile(import.meta.filename, `${sourceCode}\r\n\r\n${generatedCode.join('')}`);
      } catch (e) {
