...
export async function makeReady(): Promise<void> {
    ...
    let lastUpdate = performance.now(), startTime = lastUpdate;
    await (await getSource()).pipeTo(new UnzipStreamConsumer({
        async onFile(report, readable){
            ...
            if(lastUpdate + 200 < performance.now()){
                ...
                console.log(`📦\t...${path.slice(-25)} \tTotal Files: ${filesExtracted}  Time: ${((lastUpdate - startTime)/1000).toFixed(1)} \x1b[A`);
            }
            promises.push(new Promise(res=>Readable.fromWeb(readable).pipe(createWriteStream(path)).on("finish", res)));
        }
    }));
...
export async function getSource(): Promise<ReadableStream> {
    ...
    const latest = await fetchJson<{linux:{preview: string}, windows:{preview: string}}>("https://raw.githubusercontent.com/Bedrock-OSS/BDS-Versions/main/versions.json");
    if(!latest) {
        const readable = await getCachedBinary();
        if(readable) return readable;
    ...
    Readable.fromWeb(writeToFile).pipe(createWriteStream(file, {start: 0})).on("error", ()=>{
