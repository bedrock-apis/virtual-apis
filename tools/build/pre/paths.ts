...
const folders = (await readdir('.', { withFileTypes: true }))
   .filter(e => e.isDirectory() && !e.name.match(ignorePattern))
   .map(e => e.name);

const writable = createWriteStream('tools/types/paths.d.ts' satisfies ProjectFilePath);
writable.write(
   `type F = ${[...(await arrayFromAsync(getFilesRecursive(...folders))), ...(await readdir('.', { withFileTypes: true })).filter(e => e.isFile()).map(e => e.name)].map(e => JSON.stringify(e)).join(' | ')};\n`,
);
writable.write(
   `type D = ${[...(await arrayFromAsync(getDirectoriesRecursive(...folders))), ...folders].map(e => JSON.stringify(e)).join(' | ')};\n`,
);
writable.end(`;
declare global {
