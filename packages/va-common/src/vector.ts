...
export class VecXZ {
   ...
   public static isBetween(a: VectorXZ, b: VectorXZ, c: VectorXZ) {
      return (
         c.x >= (a.x < b.x ? a.x : b.x) &&
         c.x <= (a.x > b.x ? a.x : b.x) &&
         c.z >= (a.z < b.z ? a.z : b.z) &&
   ...
   public static center(a: VectorXZ, b: VectorXZ) {
      return {
         x: (b.x - a.x) / 2 + a.x,
         z: (b.z - a.z) / 2 + a.z,
      };
   ...
   public static equals = (a: VectorXZ, b: VectorXZ) => a.x === b.x && a.z === b.z;
   ...
   public multiplyVec(a: VectorXZ) {
      return VecXZ.multiplyVec(this, a);
   }
}

export class Vec {
   public static isVec(unit: unknown): unit is Vector3 {
    ...
    */
   public static isValid = (a: Vector3) =>
      !isNaN(a.x) &&
      !isNaN(a.y) &&
      !isNaN(a.z) &&
      typeof a.x === 'number' &&
   ...
   public static dot = (a: Vector3, b: Vector3) => a.x * b.x + a.y * b.y + a.z * b.z;

   public static equals = (a: Vector3, b: Vector3) => a.x === b.x && a.y === b.y && a.z === b.z;
    ...
    * @param z Number to increase vector on z axis. Defaults to x
    */
   public static around(a: Vector3, x: number, y = x, z = y): [Vec, Vec] {
    ...
    */
   public static *forEach(a: Vector3, b: Vector3) {
    ...
    * @returns
    */
   public static size(a: Vector3, b: Vector3) {
   ...
   public static floor(x: Vector3) {
      return new Vec(Math.floor(x.x), Math.floor(x.y), Math.floor(x.z));
   }

   /** Checks if vector c is between a and b */
   public static isBetween(a: Vector3, b: Vector3, c: Vector3) {
      return (
         c.x >= (a.x < b.x ? a.x : b.x) &&
         c.x <= (a.x > b.x ? a.x : b.x) &&
         c.y >= (a.y < b.y ? a.y : b.y) &&
   ...
   /** Returns the addition of these vectors. */
   public static add(a: Vector3, b: Vector3): Vec {
    ...
    */
   public static isInsideRadius(center: Vector3, a: Vector3, radius: number) {
      return (center.x - a.x) ** 2 + (center.y - a.y) ** 2 + (center.z - a.z) ** 2 < radius ** 2;
   }

   /** Returns the component-wise division of these vectors. */
   public static divide(a: Vector3, b: number) {
   ...
   /** Returns the component-wise product of these vectors. */
   public static multiplyVec(a: Vector3, b: Vector3) {
      return new Vec(a.x * b.x, a.y * b.y, a.z * b.z);
   }

   /** Returns the component-wise product of these vectors. */
   public static multiply(a: Vector3, b: number) {
      return new Vec(a.x * b, a.y * b, a.z * b);
   }
   ...
   public static readonly back = new Vec(0, 0, -1);

   /** A constant vector that represents (0, -1, 0). */
   public static readonly down = new Vec(0, -1, 0);

   /** A constant vector that represents (0, 0, 1). */
   public static readonly forward = new Vec(0, 0, 1);

   /** A constant vector that represents (-1, 0, 0). */
   public static readonly left = new Vec(-1, 0, 0);

   /** A constant vector that represents (1, 1, 1). */
   public static readonly one = new Vec(1, 1, 1);

   /** A constant vector that represents (1, 0, 0). */
   public static readonly right = new Vec(1, 0, 0);

   /** A constant vector that represents (0, 1, 0). */
   public static readonly up = new Vec(0, 1, 0);

   /** A constant vector that represents (0, 0, 0). */
   public static readonly zero = new Vec(0, 0, 0);
