...
export const CodeBuilder = {
  ...
  createNewCall(constructorName: string, params: ts.Expression) {
    return factory.createVariableStatement(
      [factory.createToken(ts.SyntaxKind.ExportKeyword)],
      factory.createVariableDeclarationList([
        factory.createVariableDeclaration(factory.createIdentifier(constructorName), undefined, undefined, params),
      ]),
    );
  },
};
...
const PROTOTYPE_ORIGIN_IDENTITY = i`PROTOTYPE_ORIGIN`,
  ...
  .map(data => {
            ...
            factory.createReturnStatement(
              CodeBuilder.call(
                ...
                factory.createCallExpression(
                  factory.createPropertyAccessExpression(SUPER_EXPRESSION, METHOD_IDENTITY),
                  undefined,
    ...
    return factory.createExpressionStatement(
      CodeBuilder.assign(
        ...
        CodeBuilder.call(
          OVERTAKES_IDENTITY,
          CodeBuilder.accessWith(CodeBuilder.accessWith(MODULE_NAME_IDENTITY, data.name), 'prototype'),
          factory.createObjectLiteralExpression([...METHODS, ...GETTERS, ...REQUIRED_SETS], true),
        ),
      ),
    );
  })
