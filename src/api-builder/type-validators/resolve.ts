import { Q } from 'vitest/dist/chunks/reporters.D7Jzd9GS';
import { MetadataType } from '../../package-builder/ScriptModule';
import { Kernel } from '../kernel';
import { BaseType, VoidType } from './base-types';
import { BooleanType } from './boolean-type';
import { FunctionType } from './function-type';
import { BigIntType, NumberType } from './number-type';
import { StringType } from './string-type';

export function TypeResolver(metadataType: MetadataType) {
  const { name } = metadataType;

  if (metadataType.is_bind_type) {
    const bindType = BaseType.BIND_TYPE_TYPES.get(name);
