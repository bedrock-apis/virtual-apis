...
export class KernelIterator<T> extends Kernel.Empty implements IsolatedIterator<T>{
    private constructor(src: Iterator<T>){ super(); this[ITERATOR_SOURCE_SYMBOL] = src}
    ...
    public static SetSafeIterator<T extends (this: any,...params: any[])=>Generator<S>,S>(m: T): T extends (this: infer N, ...params: infer M)=>Generator<S>?(this:N, ...params: M)=>KernelIterator<S>:never{
        Object.setPrototypeOf(m.prototype, this.isolatedGeneratorPrototype);
        return m as unknown as T extends (this: infer N, ...params: infer M)=>Generator<S>?(this:N, ...params: M)=>KernelIterator<S>:never;
    }
    public [ITERATOR_SOURCE_SYMBOL]: Iterator<T> = null as unknown as Iterator<T>;
    public [Kernel.symbolCopy.iterator](): IterableIterator<T> { return this; }
