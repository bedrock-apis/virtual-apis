...
export class APIBuilder{
     ...
     */
    public static CreateConstructor<T extends ClassDefinition<any, any>>(definition: T): T["apiClass"] {
     ...
     */
    public static CreateMethod<T extends ClassDefinition<any,any>>(definition: T, id: string){
        // Build arrow function so the methods are not possible to call with new expression
        const ctor = (that: any, params: any[])=>{
            // Check if the object has native bound
            if(!NATIVE_OBJECTS.has(that)) throw new ErrorConstructors.BoundToPrototype(ErrorMessages.BoundToPrototype("function",`${definition.classId}::${id}`));
